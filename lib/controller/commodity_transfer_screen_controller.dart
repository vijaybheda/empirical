...
class CommodityTransferScreenController extends GetxController {
  ...
  CommodityTransferScreenController();
  ...
  Future<void> assignInitialData() async {
    ...
    List<Commodity>? commoditiesList =
        await dao.getCommodityByDeliveryToFromTable(partnerID, enterpriseId!,
            currentUser.supplierId!, currentUser.headquarterSupplierId!);

    appStorage.saveCteCommodityList(commoditiesList ?? []);
    ...
    if (commoditiesList == null) {
      ...
      if (appStorage.cteCommodityList!.isEmpty) {
        debugPrint('Error loading Commodity List');
        listAssigned.value = true;
        update(['commodityList']);
        return;
      }

      for (int i = 0; i < appStorage.cteCommodityList!.length; i++) {
        List<CommodityKeywords> keywords = await dao
            .getCommodityKeywordsFromTable(appStorage.cteCommodityList![i].id!);
        ...
        appStorage.cteCommodityList![i].keywords = result;
      }
      List<Commodity> _commodityList = appStorage.cteCommodityList ?? [];
